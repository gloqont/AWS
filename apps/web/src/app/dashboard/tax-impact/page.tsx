  ...
  return (
    <div className="min-h-screen px-6 py-8">
      <div className="mx-auto max-w-6xl">
        ...
        {loading ? (
          <div className="mt-6 rounded-2xl border border-border bg-card/80 backdrop-blur p-6 text-sm text-foreground/70">
            Loading…
          </div>
        ) : !decision || !portfolio ? (
          <div className="mt-6 rounded-2xl border border-border bg-card/80 backdrop-blur p-6">
            ...
            {!decision ? (
              <p className="text-sm text-muted-foreground mt-2">
                No decision found. Run a decision in <span className="text-foreground">Scenario Simulation</span> first,
                then come back here.
              </p>
            ) : null}
          </div>
          ...
          <>
            ...
            <div className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-4">
              <div id="tax-decision-info" className="rounded-2xl border border-border bg-card/80 backdrop-blur p-5">
              ...
              <div id="tax-residency-selector" className="rounded-2xl border border-border bg-card/80 backdrop-blur p-5">
              ...
              <div id="portfolio-value-display" className="rounded-2xl border border-border bg-card/80 backdrop-blur p-5">
            ...
            <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
              <div id="estimated-tax-payable" className="rounded-2xl border border-border bg-card/80 backdrop-blur p-5">
                <div className="text-sm text-muted-foreground">Estimated Tax Payable</div>
                <div className="mt-2 text-3xl font-semibold tabular-nums">
                  ${fmtMoney(computed?.estimatedTax ?? 0)}
                </div>
              </div>

              <div id="effective-tax-rate" className="rounded-2xl border border-border bg-card/80 backdrop-blur p-5">
              ...
              <div id="after-tax-impact" className="rounded-2xl border border-border bg-card/80 backdrop-blur p-5">
                <div className="text-sm text-muted-foreground">After-Tax Impact</div>
                <div className="mt-2 text-3xl font-semibold tabular-nums">
            ...
            <div className="mt-6 rounded-2xl border border-border bg-card/80 backdrop-blur p-6">
                    ...
                    {computed ? (
                      <>
                        <tr className="border-b border-border/60">
                          ...
                          <td className="py-2 text-foreground/70">Long-term sale</td>
                        </tr>
                        <tr className="border-b border-border/60">
                          ...
                          <td className="py-2 text-foreground/70">Partial rebalance</td>
                        </tr>
                        <tr className="border-b border-border/60">
                          ...
                          <td className="py-2 text-foreground/70">Region-specific</td>
                        </tr>
                        <tr>
                          <td className="py-2">FX tax drag</td>
                          <td className="py-2">${fmtMoney(computed.portfolioValue * computed.rules.fx_drag)}</td>
                          <td className="py-2 text-foreground/70">Cross-border exposure</td>
                        </tr>
                      </>
                    ) : null}
                  </tbody>
              ...
              <div className="mt-5 rounded-xl border border-border bg-muted p-4 text-xs text-muted-foreground leading-relaxed">
                <div className="font-medium text-foreground mb-1">Evidence & assumptions</div>
