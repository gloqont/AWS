...
export default function ScenarioSimulationPage() {
  ...
  useEffect(() => {
    ...
    const hasShownScenarioTutorial = localStorage.getItem("has_shown_scenario_tutorial");

    // Auto-start if requested explicitly OR if first time user on this tab
    if (
      (tutorialParam === "scenario" && !isTutorialActive) ||
      (!tutorialParam && !isTutorialActive && !hasShownScenarioTutorial)
    ) {
      const timer = setTimeout(() => {
        localStorage.setItem("has_shown_scenario_tutorial", "true");
