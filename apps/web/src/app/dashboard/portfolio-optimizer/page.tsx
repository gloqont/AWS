...
export default function PortfolioOptimizerPage() {
  ...
  useEffect(() => {
    ...
    const maybeStartTutorial = () => {
      ...
      const force = localStorage.getItem("forcePortfolioTutorial") === "1";
      const hasCompleted = localStorage.getItem("hasCompletedTutorial") === "true";
      if (hasCompleted && !force) return;
      hasAutoStarted.current = true;
      timer = setTimeout(() => {
        if (!cancelled) {
          if (force) localStorage.removeItem("forcePortfolioTutorial");
    ...
    <div className="min-h-screen px-6 py-8">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-6 grid grid-cols-1 lg:grid-cols-4 gap-4">
          {/* Name */}
          <div id="portfolio-name-input" className="rounded-2xl border border-border bg-card/80 backdrop-blur p-5">
          ...
          {/* Risk budget */}
          <div id="risk-budget-selector" className="rounded-2xl border border-border bg-card/80 backdrop-blur p-5">
            <div className="text-sm text-muted-foreground">Risk Budget</div>
          ...
          <div id="total-portfolio-value" className="rounded-2xl border border-border bg-card/80 backdrop-blur p-5">
            <div className="text-sm text-muted-foreground">Total Portfolio Value (USD)</div>
            <div className="mt-2 text-2xl font-semibold">${totalValue.toFixed(2)}</div>
            <div className={totalValueOk ? "mt-2 text-xs text-emerald-500" : "mt-2 text-xs text-red-500"}>
              {totalValueOk ? "OK - recalculated from quantities" : "Enter quantities to calculate portfolio value"}
            </div>
          </div>

          {/* Allocation + actions */}
          <div id="total-allocation" className="rounded-2xl border border-border bg-card/80 backdrop-blur p-5">
            ...
            <div className="mt-2 flex items-end justify-between">
              ...
              <div className={sumOk ? "text-sm text-emerald-500" : "text-sm text-red-500"}>
            ...
            <div className="mt-3 flex gap-2">
              <button
                ...
                className="flex-1 rounded-xl bg-primary text-primary-foreground font-medium py-2.5 hover:opacity-90 disabled:opacity-60"
              >
              ...
              <button
                onClick={doSave}
                disabled={!canRun}
                className="flex-1 rounded-xl border border-border bg-card py-2.5 font-medium hover:bg-muted disabled:opacity-60"
              >
                Save
              </button>
            </div>
            ...
            <button
              onClick={doAnalyze}
              disabled={!canRun}
              className="mt-2 w-full rounded-xl border border-border bg-card py-2.5 font-medium hover:bg-muted disabled:opacity-60"
            >
        ...
        {/* Positions table */}
        <div id="positions-table" className="mt-6 rounded-2xl border border-border bg-card/80 backdrop-blur p-5">
                  ...
                  <tr key={i} className="border-b border-border/60">
                    <td className="py-2 pr-4">
                      <div className="relative">
                        <input
                          className="w-full rounded-xl border border-border bg-background px-3 py-2 text-foreground outline-none placeholder:text-muted-foreground focus:border-foreground/40"
                          value={r.ticker}
                          onChange={(e) => {
                        ...
                        {suggestions[i] && suggestions[i].symbol && (
                          <div className="absolute z-10 mt-1 w-full rounded-xl border border-border bg-card/95 backdrop-blur p-1 shadow-xl">
                            <div
                              className="cursor-pointer p-3 hover:bg-muted rounded-lg transition-colors"
                            ...
                            >
                              ...
                              <div className="text-xs text-muted-foreground">{suggestions[i].symbol} · {suggestions[i].exchange || "Global"}</div>
                            </div>
                          </div>
                        )}
                      </div>
